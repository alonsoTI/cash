...
//jmoreno - para validar un email
function isEmail(email) {
        email=email.toString().replace(/^\s+|\s+$/g,"");        
        var validemail = /^\w+([\.-]?\w+)*@\w+([-]?\w+)*(\.[A-Z]{2,3}){1,2}$/i;
...
function arrayCompareNum(e1,e2) {
	return e1[0] - e2[0];

}
function arrayCompareAns(e1,e2) {
    var ans1 = e1[0].toString();
    var ans2 = e2[0].toString();
    ans1 = ans1.toUpperCase();
    ans2 = ans2.toUpperCase();
	return ans1 < ans2 ? -1 : (ans1 == ans2 ? 0 : 1);

}
function arrayCompareFec(e1,e2) {
    var fec1 = e1[0].toString();
    var fec2 = e2[0].toString();
    fec1 = fec1.toUpperCase();
    fec2 = fec2.toUpperCase();
    if(fec1.length == 10)
        fec1 = fec1.substr(6, 4) + fec1.substr(3, 2) + fec1.substr(0, 2);
    else
        fec1 = 0;
    if(fec2.length == 10)
        fec2 = fec2.substr(6, 4) + fec2.substr(3, 2) + fec2.substr(0, 2);
    else
        fec2 = 0;
	return fec1 - fec2;
...
function arraySort(tableName, column, lineNumber, columnNumber) {
    ...
    //esilva - comparaciones x tipos
    obj2 = document.getElementById("comp" + column);
    var tipo_comp = obj2.innerHTML;
    if(tipo_comp == "NUM"){
        arrayToSort.sort(arrayCompareNum);
    }else if(tipo_comp == "ANS"){
        arrayToSort.sort(arrayCompareAns);
    }else if (tipo_comp == "FEC"){
        arrayToSort.sort(arrayCompareFec);
    }else{
